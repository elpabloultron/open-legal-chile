...
def planear(deteccion: Dict[str, Any], consulta: str = "") -> Dict[str, Any]:
    ...
    if deteccion["roles"]:
        # El RIT se valida acá adentro (sin herramienta que prometa traer el estado de la causa:
        # eso vive en la Oficina Judicial Virtual, y ahí no entra ningún sistema por su cuenta).
        from pjud_connector import analizar_rit

        for rol in deteccion["roles"][:3]:
            revision = analizar_rit(rol)
            if revision.get("valido") and not revision.get("jurisdiccion"):
                advertencias.extend(revision["advertencias"])
            elif not revision.get("valido", True):
                advertencias.append(f"el Rol/RIT «{rol}» no calza con el formato esperado: revisalo")
        advertencias.append(
            "el estado de la causa se consulta en la Oficina Judicial Virtual con ClaveÚnica y "
            "captcha: eso lo hace la persona, la suite no entra a un sistema con credenciales"
        )
