"""
Pruebas de integración para los 8 conectores oficiales del Estado de Chile.
...
def test_bcn_ley_chile():
    ...
    assert res.get("idLey") == 21643 or res.get("numero") == 21643 or "titulo" in res
...
def test_cne_energia():
    ...
    assert len(capacidad) > 0

def test_panel_expertos():
    client = PanelExpertosClient()
    res = client.search_dictamenes("peajes")
    assert isinstance(res, list)
